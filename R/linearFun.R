stepLinear <- function(fullData,deltaStar,tau,lambda,tol){
	...
	function(T1,T2,Q1,xGrid,vGrid,verbose=FALSE,display=FALSE){
		...
		function(Q2){
			...
			z <- .C('tmleStep',a_delta=delta,a_tau=tau,a_n=n,a_xx=x,a_vv=v,a_lambda=lambdaGrid, a_xGrid=xGrid,a_vGrid=vGrid,a_nx=nx,a_nv=nv,a_T1=T1,a_T2=T2,a_Q2=Q2, a_normGrad=normGrad,a_logL=logL,a_epsOpt=epsOpt)
...
linearPsi <- function(fullData, deltaStar,tau){
